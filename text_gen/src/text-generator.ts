...
Жёсткие правила:
  ...
  10. 8-15 якорей для типичной игры. Не перегружай — между ними будут паутины сцен.
...
export async function processOutline(
  ...
  try {
    ...
    const userMessage = `## Бриф\n${briefJson}\n\n## Профили архетипов (для контекста персонажей)\n${archetypesJson}\n\nСгенерируй скелет сюжетных якорей (только story layer, без per-LI маршрутов). Только JSON.`;
...
Твоя задача — сгенерировать **паутину исследования** между двумя якорями: DAG из описательных и навигационных сцен. Игрок исследует локацию, наблюдает атмосферу, может встретить персонажа.
...
Жёсткие правила:
  ...
  7. ОБЯЗАТЕЛЬНО: хотя бы один путь от entry к выходу БЕЗ encounter-ов (игрок может пройти мимо).
  8. Минимальные state effects: только flagSet (если нужно пометить "заходил в кафе"), никаких stateDeltas.
  9. id сцен — snake_case, читаемые ("cafe_entrance", "counter_seat", "window_view").
  10. nextSceneId: id другой сцены или null (выход к storyAnchorTo).
  11. Паутина должна ощущаться как исследование: разные уголки локации, мелкие детали, атмосфера.

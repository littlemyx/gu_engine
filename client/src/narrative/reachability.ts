 ...
 *
 * Малая стрелка (at.phase) здесь СОЗНАТЕЛЬНО не учитывается: достижимость —
 * вопрос «есть ли слот, где юнит может сыграть», а каждый слот начинается с
 * фазы 0 и вмещает PHASES_PER_SLOT разговоров. Окно фаз влияет на порядок
 * внутри слота, а не на существование такого слота; строгий разбор («сжигателя
 * фаз для late-юнита может не найтись») оставлен симуляции политик — она
 * проживает слот фазами и ловит это на реальном пуле.
 *

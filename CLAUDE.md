...
**IDE-шелл «Планирование и генерация» (`client/src/pages/studio/`)** — основной экран работы над историей, живёт на `/playground` (и на `/studio`). Меню → панель действий → три панели (иерархия · вьюпорт · инспектор) → док → статус-бар; вьюпорт переключается между «Чертёж», «Партитура», «Сценарий», «Карта мира». Инвариант: выделение чего угодно открывает соответствующий инспектор, попапов во вьюпорте нет. Данные шелл не хранит — всё выводится derive-функциями (`studio/derive/*`) из `narrative/*`; собственный стор (`studioStore`) держит только состояние интерфейса, а проектные поля шелла (выбор веток, провенанс префабов, ступень «Сценария») — `studioProjectStore`. Дизайн-система Industry: токены в `client/src/assets/css/industry.css`, атомы в `client/src/ui/`. Префабы (персонаж · мир · аудио-набор) живут отдельным стором `gu-prefab-library` и переживают смену истории. Старый плейграунд остался на `/playground-legacy` как донор ещё не перенесённых кусков.

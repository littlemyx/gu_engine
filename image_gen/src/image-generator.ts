...
function buildCharacterPosePrompt(
  ...
  parts.push(
    ...
    `Output a SINGLE PORTRAIT IMAGE (vertical 2:3 aspect ratio, taller than wide) containing exactly ONE character — one body, one head, one pair of arms, one pair of legs. The image must show this single character in a "${pose}" pose/emotion as a full-body shot, centered in the vertical frame. STRICTLY FORBIDDEN: multiple characters, duplicates, copies, turnaround sheets, character sheets, multiple views, side-by-side variations, or any layout containing more than one figure. The image contains exactly one figure. The character must be rendered on a solid bright chromakey green (#00FF00) background. The entire background must be uniform pure green with no gradients, no shadows, no environment, no ground. The character should have a thin clean white outline (2-3 pixels) separating it from the green background. Crisp sharp edges, no blur or feathering at the borders. Maintain consistent character design, proportions, and art style.`
  );
  return parts.join("\n\n");
}

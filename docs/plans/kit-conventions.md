...
Атомы кита узкие: тон, шрифт и заливка у них зашиты, а молекуле часто нужен
оттенок, которого у атома нет (пунктирная рамка у `Frame`, акцентная ступень у
`ToneSurface`, «жирный текст на светлом» у `TextLabel`). Это не дефект —
переделывать атом под каждый частный случай значит превратить его в мешок
пропсов.

Правило: атом **не править**, нужное реализовать локально в своём
`.module.css` — на тех же токенах, которыми пользуется атом. И назвать атом в
`missingAtoms` отчёта, чтобы расхождение было видно.
